...
# comments in wrapper
hoSeqLogo <- function( file.name, order, useFreqs=F, base=2, icColumnScale=TRUE,
    ...
    if( useFreqs ){
	file.freqs <- paste( file.name, freqs.ending, sep="." )
    ...
    file.conds <- paste( file.name, conds.ending, sep="." )
    conds <- Utils.readIMMs.flat( file.conds )
    if(revComp && order == 0){
      conds = list(conds[[1]][c(dim(conds[[1]])[1]:1),c(4:1)])
    }

    # read in probabilities
    file.probs <- paste( file.name, probs.ending, sep="." )
	...
	if( yaxis ){
		if( !( yaxis.transparent ) ){
			if( is.null( yat ) ){
				...
				if( plot.ylab ){
					grid.text( ylab, x=unit( -3.5, "lines" ), rot=90, gp=gpar(
					           cex=cex ) )
				}
			} else{
...
totalContributionsToInformationContent <- function( file.name, order,
                                                    useFreqs=F, base=2,
                                                    probs.ending="ihbp",
                                                    conds.ending="ihbcp",
                                                    freqs.ending="freqs" ){
...
# comments in wrapper
hoBgSeqLogo <- function( file.name, order, base=2, icColumnScale=T,
                         icLetterScale=T, xaxis=T, yaxis=T, xfontsize=15,
                         yfontsize=15, alpha=.3, probs.ending="probsBg",
                         conds.ending="condsBg", freqs.ending="freqsBg" ){
...
# ...
hoContributionsToInformationContent <- function( file.name, useFreqs=F ){

    # read in or set default background frequencies
    if( useFreqs ){
	file.freqs <- paste( file.name, "freqs", sep="." )
	freqs <- scan( file.freqs, quiet=T )
    } else{
	freqs <- rep( .25, 4 )
    }
    ...
    # read in conditional probabilities
    file.conds <- paste( file.name, "conds", sep="." )
    conds <- Utils.readIMMs.flat( file.conds )

    # read in probabilities
    file.probs <- paste( file.name, "probs", sep="." )
    probs <- Utils.readIMMs.flat( file.probs )
