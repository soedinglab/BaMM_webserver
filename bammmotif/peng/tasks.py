...
def plot_bamm_format_generic(job):
    src_dir = bamm_directory_old(job.meta_job.job_id) + '/' # Not sure, if this is needed, but the other code does it as well.
    if not os.path.exists(meme_plot_directory(str(job.meta_job.job_id))):
        os.makedirs(meme_plot_directory(str(job.meta_job.job_id)))
    ...
    for prefix in prefixnames:
        ...
        subprocess.run(cmd)
    rename_and_move_plots(src_dir, meme_plot_directory(str(job.meta_job.job_id)))
    meme_result_file_path = os.path.join(get_job_directory(str(job.meta_job.job_id)), JOB_OUTPUT_DIRECTORY, MEME_PLOT_INPUT)
    motif_ids = get_motif_ids(meme_result_file_path)
    split_meme_file(meme_result_file_path, meme_plot_directory(str(job.meta_job.job_id)))
    zip_bamm_motifs(motif_ids, meme_plot_directory(str(job.meta_job.job_id)), with_reverse=True)
